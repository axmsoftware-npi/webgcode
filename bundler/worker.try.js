        ...
        return {
         tasks: {
            createPocket: function (event) {
                require(pckt, function (pocket) {
                    pocket.createPocketWorkerSide(event);
                });
            },
            computeToolpath: function (event) {
                require(opertns, utl, function (operations, util) {
                    ...
                    function handleMissedArea(missedArea) {
                        sendMessage({
                            missedArea: missedArea.result
                        }, missedArea.transferable);
                    }
        
                    function handleLeftStock(leftStock) {
                        sendMessage({
                            leftStock: leftStock.result
                        }, leftStock.transferable);
                    }
            ...
            acceptProgram: function (event) {
                require(parsr, simulatn, utl, function (parser, simulation, util) {
                    ...
                    function createSingleFlagProgram(type) {
                        return {
                            program: new Uint8Array([type, 0, 0, 0, 0, 0, 0, 0]).buffer,
                            programID: 0,
                            operations: []
                        };
                    }
            ...
            simulateGCode: function (event) {
                require(gcodeSimulatn, function (gcodeSimulation) {
            ...
            extractContour: function (event) {
                require(THRE, slicr, function (THREE, slicer) {
                    self.postMessage({
            ...
            computeDuration: function (event) {
                require(toolpth, utl, simulatn, function (tp, util, simulation) {
                    var totalTime = 0;
